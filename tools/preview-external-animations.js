...
if (eastSequence.length) {
  ...
  makeGif(files, gif, Number(selection.animations.walk_east_loop?.fps) || EXTERNAL_WALK_DEFAULT_FPS);
...
function buildFullSequence(direction, mirrored) {
  const idle = idleFrameByKey.get("walk_east_start");
  ...
  const frames = [];
  if (idle && existsSync(idle)) frames.push({ file: idle, mirrored });
